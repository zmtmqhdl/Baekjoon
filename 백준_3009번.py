...
for i in range(3) : # 반복문 for
    if x_list.count(x_list[i]) == 1: # 가정문 if -> count를 사용하여 x_list가 가진 원소 중 개수가 1개인 것을 찾아냄
        result_x = x_list[i]
    if y_list.count(y_list[i]) == 1: # 가정문 if -> count를 사용하여 y_list가 가진 원소 중 개수가 1개인 것을 찾아냄
        result_y = y_list[i]
